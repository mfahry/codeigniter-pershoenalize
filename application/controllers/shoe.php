...
class Shoe extends CI_Controller {
	...
	function add_process()
	{
		...
		else
		{
			...
			$this->load->model("shoe_model","",TRUE);
			$this->shoe_model->add($shoe_name, $shoe_path);
			
			$shoe_id = $this->shoe_model->max();
			header("Location: ".base_url("part/add?shoe_id=".$shoe_id));
		}
	}
	...
	{
		$data["page"] = $this->load->view("backend/add_all_data","",TRUE);
		$data["active"] = "add product";
		$this->load->view("backend/dashboard",$data);
	}
	
	function view_primary_all_data()
	{
		$data["page"] = $this->load->view("backend/list_primary_all_data","",TRUE);
