<script>
	$(document).ready(function(){
		$("#materials #list-material a").click(function(){
	...
	function paging(startWith, limit, dataUrl){
		$("#materials").hide();
		$("#materials").load(dataUrl+"?start_with="+startWith+"&limit="+limit);
		$("#materials").show(1000);
	}
</script>
<div id="list-material">
		...
		<table class="table" width="100%">
			<thead>
				<tr>
					...
					<th style="color:rgba(30, 135, 75, 0.95); font-size:15px; font-family:'Broadway'"><center>MATERIAL NAME</center></th>
				...
				if($material->num_rows() > 0)
				{
					...
					{ ?>
						<tr style="font-family:Candara; font-size:13px; padding:5px;">
							...
							<td align="center"><a rel="<?php echo base_url("material/edit?material_id=".$row->material_id); ?>" class="btn btn-warning"><span class="glyphicon glyphicon-pencil"></span> Edit</a></td>
							<td align="center"><a rel="<?php echo base_url("material/delete?material_id=".$row->material_id); ?>" class="btn btn-danger" title="delete"><span class="glyphicon glyphicon-remove"></span> Delete</a></td>
						</tr>	
					<?php
					}
				} 
	...
	<div class="text-center bg-warning" style="margin:10px 0px; padding:10px;s">
		<a rel="<?php echo base_url("material/add"); ?>" class="btn btn-success"><span class="glyphicon glyphicon-plus"></span> Add data</a>
	</div>
</div>
<ul class="pager">
	<?php
	if($count <= 10){}	
		...
		<li class="previous" style="cursor:pointer;"><a onclick="paging('<?php echo ($start_with-10); ?>','10','<?php echo base_url("material/view")?>')">&larr; Older</a></li>
		<li class="next" style="cursor:pointer;"><a onclick="paging('<?php echo ($start_with+10); ?>','10', '<?php echo base_url("material/view")?>')">Newer &rarr;</a></li>
	<?php
	}
